...
class NNDeterministicLookup(ConditionalGenerativeModel):
    ...
    def sample(self, h_world: np.ndarray, c_world: np.ndarray, n_samples: int, rng: np.random.Generator, sampling_temperature: float = 0) -> np.ndarray: # c_world included for consistency with other models
        if self._nn is None or self._h_train is None or self._q_train is None:
            raise RuntimeError("Call fit() before sample()")
        _, idcs = self._nn.kneighbors(h_world, return_distance=True) # idcs: [B,1]
        idcs = idcs[:, 0] # B
        q_world_nn = self._q_train[idcs] # [B, dQ]
        # repeat deterministically along the sample axis to match [B, n_samples, dQ]
        out = np.repeat(q_world_nn[:, None, :], repeats=n_samples, axis=1).astype(np.float32)
        return out
...
class KNNConditionalSampler(ConditionalGenerativeModel):
    ...
    def sample(self, h_world: np.ndarray, c_world: np.ndarray, n_samples: int, rng: np.random.Generator, sampling_temperature: float = 0) -> np.ndarray:
        if self._nn is None or self._h_train is None or self._q_train is None:
            raise RuntimeError("Call fit() before sample()")
        dists, idcs = self._nn.kneighbors(h_world, return_distance=True) #[B, k], [B, k]
