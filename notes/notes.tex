...
Defining some math: Let $R(\alpha) \in \mathbb{R}^{2 \times 2}$ be planar rotation:\[R(\alpha) = \begin{bmatrix}
    \cos \alpha & - \sin \alpha \\ \sin \alpha & \cos \alpha
\end{bmatrix}\]
Represent a planar pose $(p, \alpha)$ with $p \in \mathbb{R}^2$ as the following transform: \[T(p, \alpha) = \begin{bmatrix}
    R(\alpha) & p \\ 0 \quad 0  & 1 
\end{bmatrix} \in SE(2)\]
Transform inverse:
...
Using the introduced notation, we define the frame of the base in the world as:\[T_{WB} = T\left(\begin{bmatrix}
    x \\ y
...
\[p_{BE} = p_BE(\theta), \quad \phi_{BE} = \phi_{BE}(\theta) = \theta_1 + \theta_2 + \theta_3\]
