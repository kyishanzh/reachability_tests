...
\textcolor{red}{Clean up the math in this section! Switch all the notation to use 6.4210 transform notation, and re-solve out the formulas cleanly. Also add notes on the rejection sampling / self-collision check we've implemented in the code.}
...
\begin{itemize}
    \item Base configuration: $(x_B, y_B, \psi)$
    \item Arm configuration: $\theta = (\theta_1, \theta_2, \theta_3)$
    \item Target pose: $H = (h_x, h_y, \phi)$
\end{itemize}
Defining some math: A planar pose $(x, y, \alpha)$ is represented by the matrix $T \in SE(2)$ (the homogeneous transform):
\[T(x, y, \alpha) = \begin{bmatrix}
    \cos \alpha & - \sin \alpha & x \\ \sin \alpha & \cos \alpha & y \\ 0 & 0 & 1
...
Using the introduced notation, we define the frame of the base in the world as:\[^W T^B = T\left(\begin{bmatrix}
    x \\ y
...
\begin{center}
    \textcolor{cyan}{\textsc{The Pipeline}}
\end{center}
...
Pipeline:
\begin{enumerate}
    \item Sample arm angles: $\theta = (\theta_1, \theta_2, \theta_3) \sim \mathcal{D}_\theta$
    \item Compute FK in base frame:\[p_{BE}(\theta) = \begin{bmatrix}
        \sum_{i=1}^3 L_i \cos \left(\sum_{j=1}^i \theta_j\right) \\
        \sum_{i=1}^3 L_i \sin \left(\sum_{j=1}^i \theta_j\right)
    \end{bmatrix}, \qquad \phi_{BE}(\theta) = \theta_1 + \theta_2 + \theta_3\]
    \item Sample a target pose: $H = (h_x, h_y, \phi) \sim \mathcal{D}_h$. Define $p_H = [h_x, h_y]^\top$. 
    \item Solve base pose by inversion:\[\psi = \text{wrap}_{2\pi} (\phi - \phi_{BE}(\theta)), \quad \begin{bmatrix}
        x \\y
    \end{bmatrix} = p_H - R(\psi) p_{BE}(\theta)\]
    \item Final configuration vector: $Q = (x, y, \psi, \theta_1, \theta_2, \theta_3)$. Store the pair $(Q, H)$, and repeat until you have $N$ samples. 
\end{enumerate}
...
\subsection{Evaluation metrics} \textcolor{blue}{Come up with new evaluation metrics now that we have more joint links! (e.g. MMD, EMD, etc.)}
...
In addition to the ones introduced in section \ref{eval simple}, we add the following metrics:
